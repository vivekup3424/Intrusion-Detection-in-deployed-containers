...
class CCA:
    ...
    def robust_cca_similarity(self, acts1, acts2, threshold=0.98, epsilon=1e-6,
                              compute_dirns=True, num_cca_trials=5):
    ...
    def get_cca_similarity(self, acts1, acts2, epsilon=0., threshold=0.98,
                           compute_coefs=True,
                           compute_dirns=False,
                           verbose=True):
        ...
        ([u, s, v], invsqrt_xx, invsqrt_yy, x_idxs, y_idxs) = CCA.compute_ccas(sigmaxx, sigmaxy, sigmayx,
                                                                               sigmayy,
                                                                               epsilon=epsilon,
                                                                               verbose=verbose)
        ...
        if (not np.any(x_idxs)) or (not np.any(y_idxs)):
            return CCA.create_zero_dict(compute_dirns, acts1.shape[1])

        if compute_coefs:

            # also compute full coefficients over all neurons  # x_idx = [True,...True] len(x_idx)=100,
            # y_idx = [True,...True] len(y_idx)=50
    ...
    @staticmethod
    def compute_ccas(sigma_xx, sigma_xy, sigma_yx, sigma_yy, epsilon):  # epsilon = 0
